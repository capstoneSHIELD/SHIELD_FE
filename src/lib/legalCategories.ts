...
export interface CategorySelection {
  name: string;
  path: string[]; // 길이 1=대분류, 2=중분류, 3=소분류
}

/** 검색 결과 */
export interface CategorySearchResult {
  leaf: CategoryLeaf;
  path: [string, string, string]; // [대분류, 중분류, 소분류]
}

// ── Data ───────────────────────────────────────────────────────────────────

export const LEGAL_CATEGORY_TREE: CategoryNode[] = [
  {
    name: '부동산 거래',
    children: [
      {
...
export function searchCategories(
  query: string,
  data: CategoryNode[] = LEGAL_CATEGORY_TREE,
...
export function buildLeafPathMap(
  data: CategoryNode[],
  ...
  for (const l1 of data) {
    for (const l2 of l1.children) {
      for (const leaf of l2.children) {
        map.set(leaf.name, [l1.name, l2.name, leaf.name]);
      }
    }
  }
